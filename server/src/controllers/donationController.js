...
async function screenDonationProof(
  buffer,
  mimeType,
  claimedAmount,
  fundraiserName,
...
// виписці не одразу.
async function checkBankConfirmation(
  fundraiser,
  session,
  sessionId,
  claimedAmount,
  ...
  try {
    ...
    const statement = await fetchJarStatement(
      fundraiser.monobankJarId,
      from,
      to,
    );
    ...
    const txText = (tx) =>
      `${tx.description || ""} ${tx.comment || ""}`.toUpperCase();
  ...
  try {
    ...
    if (bankConfirmed) {
      await createNotification({
        ...
        message:
          "Банк автоматично підтвердив переказ - сесію підтверджено, очікуй на зустріч.",
        link: `/dashboard`,
      });
...
exports.attachDonationProof = async (req, res) => {
  try {
    ...
    if (donation.proofUrl) {
      return res
        .status(409)
        .json({ message: "Скрін для цього донату вже додано" });
    }
    ...
    if (existingByHash) {
      return res.status(409).json({
        message:
          "Цей файл підтвердження вже використовувався для іншого донату",
      });
    }
    ...
    try {
      ...
      extractedReference =
        result.referenceFound && result.extractedReference
          ? result.extractedReference
          : null;
      extractedRecipient =
        result.recipientFound && result.extractedRecipient
          ? result.extractedRecipient
          : null;
      ...
      console.error(
        "⚠️ AI-скринінг донату не вдався (скрін все одно додаємо):",
        aiError,
      );
    }
    ...
    if (bankConfirmed) {
      await createNotification({
        ...
        title: "Донат підтверджено",
        message:
          "Банк підтвердив переказ - сесію підтверджено, очікуй на зустріч.",
        link: `/dashboard`,
      });
    } else {
      const specialistProfile = await prisma.specialistProfile.findUnique({
    ...
    const withPresignedUrls = await Promise.all(
      donations.map(async (donation) => ({
        ...
        proofUrl: donation.proofUrl
          ? await getPresignedUrl(donation.proofUrl)
          : null,
      })),
    );
